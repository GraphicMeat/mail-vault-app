...
`tests/unit/vaultInDaemon.test.js`'s `ALLOWED_WRITERS` (now `backup.rs`, `commands.rs`, `main.rs`) keeps the real list closed. Since Task 3.9 its pattern also sees a raw `fs::write` / `fs::copy` into a vault path, not only a `vault_files::` call - neither the mbox importer nor the Graph backup writer was ever routed through `vault_files::`, so the old pattern was structurally blind to both. Backup's own migration is deferred, blocked on a security-scoped bookmark probe Rokas has not yet run (see Known Gaps below); until it moves, this paragraph keeps its caveat for `backup.rs`, `commands.rs`, `main.rs`'s mbox importer, and the app-invoked half of `archive.rs`.
...
- **Backup stays in the app.** `backup.rs` (mirror sync, Graph/IMAP importers) is unmoved and ungated, deferred until Rokas runs the security-scoped bookmark probe (ledger `docs/superpowers/ledgers/2026-09-16-daemon-shell-phase3/progress.md`, "Backup bookmark scope probe"). Its bookmark lifetime already diverges from the three `vault_flags.rs` forwarders (spans the whole run, not one call); moving it has to resolve that too, not just relocate the code.
- **Two `ImapPool`s per account until Phase 5.** The daemon's archive/bulk routes use the daemon's own pool; the app's pool still serves interactive IMAP, up to 10 concurrent connections per account combined (two 5-permit semaphores). Marked with a `ponytail:` comment at the daemon route.
...
- **The archived-ids map has more bypass writers than Task 3.8 closed.** `messageListSlice.js`'s `_archivedIdsByGroup` is fed by 5 named writers, plus 3 more (`activateAccount.js`, `AccountPipeline.js`, `messageMutations.js`) fixed in a follow-up round after review found they could stale-narrow the map. `localDrafts.js` and a further 4-6 sites (`loadEmails.js` x2, `messageMutations.js` x2, others) still write `archivedEmailIds` directly on a successful read without feeding the map - the same latent-stale-map shape, not yet fixed. Tracked separately as background task `80ba3da8`, not part of this branch.
