...
- **Keeping the daemon alive across app quit and login** (`src-tauri/src/autostart.rs`, pure parts in `mailvault_core::autostart`). One setting, `daemonAlwaysOn`, drives two things: `RunEvent::Exit` skips the daemon SIGTERM so the running daemon is adopted by init, and a per-platform login item brings it back at login. `stop_daemon()` (vault moves, restarts) is unaffected; only the app-quit path is gated.
  - macOS registers through `SMAppService`. Developer ID registers the *bundled* LaunchAgent (`Contents/Library/LaunchAgents/com.mailvault.app.daemon.plist`, `BundleProgram` so it follows the app wherever it is dragged). **The agent label must sit under the app's bundle id** (`com.mailvault.app.daemon`, not `com.mailvault.daemon`): SMAppService inherits the SMLoginItem rule that a helper's identifier is prefixed by the main app's, and a label that breaks it is answered with `NotFound` — indistinguishable, from the API alone, from a plist that is not in the bundle at all. That is why `NotFound` is only reported as "unsupported" when the plist is genuinely absent; when it is present the switch stays live so `register()`'s NSError can surface. Registered, launchd runs the daemon itself with no UI. The App Store build cannot: its sidecar is signed `app-sandbox` + `com.apple.security.inherit`, and a binary with `inherit` aborts unless its sandboxed parent spawned it. Under `feature = "appstore"` it registers the app instead, which spawns the daemon as before. The plist is signature-sealed, so it is shipped, never written at runtime; a core test holds the shipped copy to the one definition.
