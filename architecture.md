...
- **The keychain gate** (`credentials.rs`, classifier in `mailvault_core::keychain`). A daemon keychain read that fails for a reason unlocking would fix (locked, a refused or cancelled prompt, or no answer within the read's clock) blocks that item; the next read of the same item that answers, even with "nothing stored", clears it, and the gate is blocked while any item is. Each item has at most one blocking read out at a time, which later callers join rather than start again, so a read parked on an unanswered prompt cannot multiply into a thread per retry. The daemon does not wait to be asked: a watcher task reads the blob once at start, and while the gate is blocked reads every blocked item again every 5 seconds (never a refused one, which would re-prompt), sleeping while it is clear; the clear wakes the scheduled-send worker. Each blocked/clear flip is one `keychain-status` event, and `keychain.status` answers the current state for an app that connects later. On macOS the app shows a corner card for as long as the gate is blocked, with no dismiss. Its Unlock calls `keychain.retry` first, which reads (or joins the read already out for) both daemon items under a 120s clock and wakes the scheduled-send worker when they answer; only when that says `locked` does the app do its own foreground read (a locked keychain prompts a foreground app, not a background one) and retry once more. Every blocked-to-clear flip, whatever caused it, runs the app's `retryKeychainAccess` once so its own "Password missing" state recovers and `sync.watch` registers again. While blocked, scheduled sends stay queued without spending a try, like offline, and the worker looks again at most every 5 minutes on its own. When a blocking transition reaches no receiver, the daemon posts a banner itself through UNUserNotificationCenter (at most once per 30 minutes); that relies on `NSBundle.mainBundle` resolving to `MailVault.app` from `Contents/MacOS` and is unproven until a signed build shows it.
