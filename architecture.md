...
- **A cancelled Graph backup releases the mirror scope late.** `run_graph_account`'s cancel path breaks out of the folder loop without emitting a terminal `backup-progress` frame (pre-existing behavior, inherited verbatim from the app-side runner it replaced), and the daemon only synthesizes one for an early `Err`. The shell therefore keeps that account's scope parked until the account's next run displaces it (which releases the stale one). Same for a Graph account with zero folders. The fix belongs in the runner's cancel path, not in the shell's release hook.
