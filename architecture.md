...
The shell must not do heavy work: no transport, no vault walks, no indexing, no bulk file I/O. A Tauri command is a forwarder to a daemon RPC, not an implementation. Apart from the settings file and the three bookmark forwarders above, the shell does not open or write a vault file, cache, journal or ledger directly, with one recorded exception: `archive.rs`, `backup.rs`, `commands.rs`, `restore.rs` and `main.rs` (its mbox importer) still call into `mailvault_core::vault_files`/`vault_flags` directly, unmoved by Phase 2 and not behind the daemon's vault gate, so a vault move can race any of them. `tests/unit/vaultInDaemon.test.js`'s `ALLOWED_WRITERS` keeps this list closed so it cannot grow by accident. Phase 3 moves archive and backup into the daemon; Phase 4 moves restore and the mbox import; once all five are gone this paragraph goes back to an unqualified "does not write directly".
