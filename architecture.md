...
Account transfer (Settings > Accounts, "Move to another computer") is split along the same line. The daemon owns the file format and the crypto: `transfer.export` encrypts a bundle the app hands it, `transfer.decrypt` opens one, and `transfer.apply_config` merges the bundle's app.db config (tags, fields, views, auto-tag rules) under an account id map. A `.mvtransfer` file is `MVXFER`, a version byte, the argon2id parameters (export: m=64 MiB, t=3, p=1), a 16-byte salt and a 24-byte nonce, then XChaCha20-Poly1305 ciphertext with the whole header as AAD (`mailvault_core::transfer::crypto`). The daemon never receives a user-chosen path: the app picks the file through the native dialog, writes it with `save_attachment_to`, and reads it with `read_file_base64`, a thin Tauri command that only opens `.mvtransfer` files. The app remains the only writer of account credentials: `saveAccounts` (`services/db/accounts.js`) stores every imported account in one keychain write and one accounts.json write, and frontend settings travel through an allowlist (`services/transfer/settingsTransfer.js`) that leaves machine-bound state behind. The only secret the daemon writes is the AI provider key, into the keychain entry it already owns. Import runs in the main window only (`AccountImportModal`, raised through `settings-window-action` from a detached Settings window), because the detached window cannot write settings to disk and the main window's keychain cache would drop the imported secrets on its next write; the main window reloads when the import lands.
