...
- **The keychain gate** (`credentials.rs`, classifier in `mailvault_core::keychain`). A daemon keychain read that fails for a reason unlocking would fix (locked, a refused or cancelled prompt, or no answer within the read's clock) blocks that item; the next read of the same item that answers, even with "nothing stored", clears it, and the gate is blocked while any item is. Each item has at most one blocking read out at a time, which later callers join rather than start again, so a read parked on an unanswered prompt cannot multiply into a thread per retry. Each blocked/clear flip is one `keychain-status` event, and `keychain.status` answers the current state for an app that connects later. On macOS the app shows its unlock dialog, does its own foreground read (the prompt macOS shows a foreground app), then calls `keychain.retry`, which reads (or joins the read already out for) both daemon items under a 120s clock and wakes the scheduled-send worker when they answer. While blocked, scheduled sends stay queued without spending a try, like offline, and the worker looks again every 5 minutes. When a blocking transition reaches no receiver, the daemon posts a banner itself through UNUserNotificationCenter (at most once per 30 minutes); that relies on `NSBundle.mainBundle` resolving to `MailVault.app` from `Contents/MacOS` and is unproven until a signed build shows it.
