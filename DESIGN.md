...
Appearance → Layout also offers Backup status location: On avatar (default), End of row, or Hidden, with a live sample. The avatar badge uses a muted check for healthy backups and amber for warnings, and occupies no separate account-name column. End of row retains the green healthy icon. The collapsed rail keeps the badge away from connection status. Placement is saved independently of backup schedules; hiding the icons leaves backups and progress notifications running. Every visible status remains a separate, keyboard-accessible button that opens backup details for its account.
