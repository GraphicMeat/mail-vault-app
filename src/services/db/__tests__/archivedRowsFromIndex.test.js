 ...
 * tier now reads `vault_rows` and no cache is written at all. The files tier
 * reads the vault registry's stored light rows (`vault_light_rows`).
 *
...
const DEFAULTS = {
  ...
  // The registry's light rows: uids the vault does not hold are left out.
  vault_light_rows: (args) => args.uids.filter((uid) => uid === 10).map((uid) => ({ uid, subject: 'from eml', snippet: '' })),
};
const ROWS = { ...DEFAULTS };
const calls = [];
...
describe('getArchivedEmails builds rows from sidecars, then the index, then the files', () => {
  ...
  it('asks the index only for what the sidecars missed, and the files only for what the index missed', async () => {
    ...
    expect(calls.find((c) => c.cmd === 'vault_light_rows').args.uids).toEqual([10]);
  ...
  it('a failing index read is not a missing row: the files fill in', async () => {
    ...
    ROWS.vault_light_rows = (args) => args.uids.map((uid) => ({ uid, subject: `eml ${uid}` }));
    const rows = await getArchivedEmails('acct', 'INBOX', new Set([10, 20]));
    expect(rows.map((r) => r.uid).sort()).toEqual([10, 20]);
  });

  it('an unknown files tier (`null`) stops there and keeps the rows already built', async () => {
    const { getArchivedEmails } = await import('../emails.js');
    ROWS.vault_light_rows = () => null;
    const rows = await getArchivedEmails('acct', 'INBOX', new Set([10, 20, 30]));
    expect(rows.map((r) => r.uid)).toEqual([30, 20]);
  });

  it('asks the files tier 500 uids per call', async () => {
    const { getArchivedEmails } = await import('../emails.js');
    ROWS.load_email_cache_by_uids = () => [];
    ROWS.vault_rows = () => [];
    ROWS.vault_light_rows = () => [];
    await getArchivedEmails('acct', 'INBOX', new Set(Array.from({ length: 1200 }, (_, i) => i + 1)));
    expect(calls.filter((c) => c.cmd === 'vault_light_rows').map((c) => c.args.uids.length)).toEqual([500, 500, 200]);
  });
