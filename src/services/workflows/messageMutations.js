...
export function indexEntryFor(email, extra = {}) {
  return {
    ...
    date: email.date,
    provider: email.provider ?? (email.source === 'graph' ? 'graph' : null),
    ...(Object.hasOwn(email, 'messageDate') ? { messageDate: email.messageDate } : {}),
    receivedAt: email.receivedAt ?? email.internalDate ?? null,
