...
// `scan` is the backup-scan state read once when the search started.
...
export const useSearchStore = create((set, get) => ({
  ...
  performSearch: async () => {
    ...
    const { emails, localEmails, activeMailbox, activeAccountId, accounts, savedEmailIds, mailboxes,
      backedUpKeys, backedUpScopes, backupConfigured } = useMailStore.getState();
    const scan = { backedUpKeys, backedUpScopes, backupConfigured, activeAccountId, activeMailbox };
    ...
    try {
      ...
      if (searchFilters.location !== 'local' && account && hasValidCredentials(account)) {
        ...
        for (const [i, mailboxToSearch] of targets.entries()) {
          ...
          if (targets.length > 1) {
            if (superseded()) return;
            set({ searchResults: finalize(allResults, scan), searchProgress: { done: i + 1, total: targets.length } });
          }
        }
      }

      const deduplicatedResults = finalize(allResults, scan);
