...
import { _buildRestoreDescriptor, _resolveUnifiedContext, _selKey, _parseSelKey, _resolveMailboxPath } from './unifiedHelpers';
import {
...
export const createAccountSlice = (set, get) => ({
  ...
  // Initialize
  init: async () => {
    // We need a reference to useMailStore for module-level functions.
    // Since the store is created after this function, we use get/set from the closure.
    const useMailStoreRef = { getState: get, setState: set };
  ...
  // ── Workflow actions (archive, delete, mark read, etc.) ──
