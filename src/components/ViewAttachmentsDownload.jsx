...
export function ViewAttachmentsDownload({ view, rows, name }) {
  ...
  const run = async choice => {
    ...
    try {
      ...
      setState({ done: result?.files ? t('views.download.done', { count: result.files }) : t('views.download.none') });
  ...
  return <>
    ...
    <Popover open={!!menu} onClose={() => setMenu(null)} role="menu" aria-label={t('views.download.title')}
