      ...
      <div className="p-6">
        <div className="bg-mail-surface border border-mail-border rounded-xl p-8 text-center">
          ...
          <p className="text-xs text-mail-text-muted max-w-md mx-auto">
            {t('settings.ai.localAiPoweredEmailClassification')}
          </p>
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="bg-mail-surface border border-mail-border rounded-xl p-5">
        <div className="flex items-center gap-3 mb-3">
          ...
          <h3 className="text-sm font-semibold text-mail-text">{t('settings.ai.howEmailCleanupWorks')}</h3>
        </div>
        <p className="text-xs text-mail-text-muted mb-3">
          {t('settings.ai.emailCleanupUsesLocalAi')}
        </p>
        <ol className="text-sm text-mail-text-muted list-decimal list-inside space-y-1.5">
          <li>{t('settings.ai.mailvaultAnalyzesSenderDomainsSubject')}</li>
          <li>{t('settings.ai.eachEmailGetsCategorySuggested')}</li>
          <li>{t('settings.ai.whenCorrectClassificationMailvaultLearns')}</li>
          <li>{t('settings.ai.canReviewManageLearnedRules')}</li>
        </ol>
      </div>
      ...
      <div className="bg-mail-surface border border-mail-border rounded-xl p-5">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-sm font-semibold text-mail-text">{t('settings.ai.learnedRules')}</h3>
              <p className="text-xs text-mail-text-muted">{t('settings.ai.autoGeneratedCorrections')}</p>
            </div>
          </div>
          <div className="flex gap-1.5">
            ...
            >
              <FileDown size={12} /> {t('common.export')}
            </Button>
            <Button variant="secondary" size="xs" className="bg-transparent text-[11px] px-2.5"
              onClick={handleImportRules}
            >
              <Upload size={12} /> {t('settings.ai.import')}
            </Button>
          </div>
        </div>
        ...
        {rules.length === 0 ? (
          <p className="text-xs text-mail-text-muted text-center py-4">
            {t('settings.ai.noRulesYetRulesAuto')}
          </p>
        ) : (
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      ...
                      <label className="space-y-0.5">
                        ...
                        <select value={editForm.action} onChange={e => setEditForm(f => ({ ...f, action: e.target.value }))}
                          ...
                          <option value="archive">{t('common.archive')}</option>
                          <option value="delete-from-server">{t('common.delete')}</option>
                          <option value="review">{t('settings.ai.review')}</option>
                        </select>
                      </label>
                    </div>
                    <button
      ...
      <div className="bg-mail-surface border border-mail-border rounded-xl p-5">
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            <h3 className="text-sm font-semibold text-mail-text">{t('settings.ai.customCategories')}</h3>
            <p className="text-xs text-mail-text-muted">{t('settings.ai.addOwnCategoriesClassification')}</p>
          </div>
        </div>

        <div className="space-y-2 mb-4">
          {customCategories.length === 0 ? (
            <p className="text-xs text-mail-text-muted italic">{t('settings.ai.noCustomCategoriesYet')}</p>
          ) : (
        ...
        <form onSubmit={async (e) => {
          ...
          <div className="flex gap-2">
            ...
            <button
              ...
              title={t('settings.ai.addRuleCategory')}
            >
              <Plus size={14} />
            </button>
            <button
