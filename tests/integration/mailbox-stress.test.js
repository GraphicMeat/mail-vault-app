...
describe('Mailbox Stress Test (500 emails → local storage → server delete → verify)', { timeout: 600000 }, () => {
  ...
  it('should fetch and save all 500 emails to local storage', async () => {
    ...
    expect(localStorage.size).toBe(testUids.length);
  });
  ...
  it('should have correct content in local storage', () => {
    // Check email type distribution: 5 types cycling — each should be ~1/5 of total
    ...
    // Each type should have at least some emails (proportional to total appended)
    const total = plain + html + multipart + attachment + multiAtt;
    expect(total).toBe(localStorage.size);
  ...
  // -----------------------------------------------------------------------
  it('should still have all emails in local storage after server deletion', () => {
    expect(localStorage.size).toBe(testUids.length);
  ...
  it('should be able to search local storage by subject, sender, and content', () => {
    ...
    expect(byBody.length).toBeGreaterThan(0); // only 'multipart' type
    ...
    expect(withJson.length).toBeGreaterThan(0); // only 'multi-attachment' type
  });
  ...
  it('should handle bulk delete from local storage', () => {
    ...
    const sizeBefore = localStorage.size;
    ...
    expect(localStorage.size).toBe(sizeBefore - plainIds.length);

    // Delete all html-only emails
    const sizeAfterPlain = localStorage.size;
    ...
    expect(localStorage.size).toBe(sizeAfterPlain - htmlIds.length);

    // The remaining should be multipart + attachment + multi-attachment
