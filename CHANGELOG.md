...
- **Messages can be starred, and the star follows you to every other client on the account.** Hover a row for the star, or use the one in the reading pane, the row menu, or the `s` key. A starred message is starred in webmail and on your phone too, because MailVault now writes the flag to the server the way it always wrote "read". Replying marks the original as answered on the server, and forwarding marks it forwarded, so other clients show the little arrows.
- **Folders can be created, renamed and deleted from the sidebar.** Right-click a folder for New subfolder, Rename and Move to Trash; the "+" next to Folders makes a top-level one. Deleting a folder files it under Trash, the way Thunderbird does, and only a folder already inside Trash is deleted for good, after a confirmation. Your vault copies follow a renamed folder to its new name; nothing local is ever deleted. INBOX and the special folders (Sent, Drafts, Trash, Junk, Archive) cannot be renamed or deleted.
- **New mail arrives as it lands, not on the next timer tick.** MailVault now keeps one connection per account listening on the inbox (IMAP IDLE), so a new message shows up and notifies within a second or two. The five-minute refresh stays as the fallback for servers without IDLE and for the other folders.
- **Undo for moves, Trash deletes, stars and read marks.** Every one of those actions offers Undo in a small toast for eight seconds, and ⌘Z brings back the last one at any time until the next action replaces it. A permanent delete says so and offers nothing.
- **Changes made offline are kept and sent when the connection returns.** Marking read, starring, moving and deleting while offline apply on screen immediately and are queued; MailVault sends them when the network is back (or on the next launch), the same way it already finished deletes interrupted by a quit.
...
- **Move to folder works again for a message found by search.** Moving a message that search had found in another folder — an INBOX message while another folder was on screen — failed with "invalid args `uids` … expected u32". The folder list was handed the message's selection key instead of its number, and the server command wanted the number. Every message now moves out of the folder it actually lives in, whether it came from a search, a branch listing or a merged Sent copy, and a moved message leaves the search results instead of lingering there.
- **Menus and popovers no longer run off the bottom of the window.** The row menu opened on a row near the bottom edge, and the Move to folder list it opens beside it, could end below the window, and the only way to reach the last items was to scroll the list up and try again. Every floating panel now measures itself and shifts to stay inside the window — row menus, the folder list, the sender and authentication popovers, the copy menu inside a message — and a dialog taller than the window scrolls instead of being cut off.
- **Open Folder under Backup Scope & Storage opens the folder.** The backup folder lives outside the app's sandbox, and the app only held permission to it while validating or backing up, so a click landing in between was refused by macOS and nothing happened — not even an error. The button now takes that permission for the click and reports a refusal instead of swallowing it. The backup settings also name the vault's real folder once the vault has been moved.
- **A dropped connection during sync no longer empties a folder's cached list.** When a pooled connection died, the first sync tick failed with "SELECT INBOX failed: io: Broken pipe" — and when the server closed the socket cleanly, the reply read as an empty mailbox and the sync pruned every cached header to match, logged as "Sync complete: 0 total"; one such prune removed 1,399 INBOX headers. A reply with nothing in it is now treated as a lost connection, and sync and backfill retry once on a fresh connection.
- **The offline banner stays up while the Mac has no network.** With Wi-Fi off and the app idle, the banner appeared and then cleared within 30 seconds, because the sync service's heartbeat reported "online" without ever having tried. The system's own link state now outranks that untested answer.

## [2.11.1] - 2026-09-03

### Added
- **Folders are shown the way your server files them.** A nested archive — Clients, a folder under it for each company, a Done under each of those — was listed in the sidebar as one flat alphabetical list, so a reader with ten folders called Done saw ten identical rows and nothing to tell them apart. The hierarchy was never missing from what the server sends; it was simply not drawn. The sidebar now shows folders as a tree: a folder with folders inside it carries a chevron, opening it shows what is filed beneath, and MailVault remembers which folders you left open, for each account separately. Reading a message that sits five levels down — a search hit, say — opens the folders above it on its own. The Move to folder list shows the same tree, where every folder was previously drawn at the top level under its own name alone; while you are typing a filter there the parents are filtered away, so each row names its parent instead. The Tag Cloud sidebar style nests too: a chip carries its own name and, when something is filed inside it, the same chevron; open it and its children hang beneath it while the chips after it carry on in a new row. Two rules keep the top of the tree useful: a server that files every folder under the Inbox no longer prints that prefix on each one, and the special folders a provider keeps inside a container — Gmail's [Gmail] — are listed at the top beside the Inbox rather than a level down.
- **Open a folder that has folders inside it and you see everything filed under it, and a search can be told to look there and nowhere else.** Clicking a parent folder showed only the messages in that folder itself, which for a folder that exists to hold other folders is usually nothing. It now lists the whole branch — every message in that folder and in every folder beneath it — newest first, with the total the server reports for the branch, and the list fills in folder by folder rather than all at once at the end, because a twenty-folder branch takes long enough that a list arriving only at the end reads as a folder that found nothing. A folder that will not open costs you that folder alone; its siblings still list. The same message number in two folders stays two rows. The search's folder filter, which offered one folder or every folder on the server, gained an "Include subfolders" box: tick it and the search covers the chosen folder and its whole branch, on the server and in your vault, and the results header says so. "All folders" still keeps a saved message from a folder the server no longer lists, because that mail is still readable.
- **When the internet goes away, MailVault says so and stops trying to sync until it is back.** A machine with no Wi-Fi reported "The server refused the sync", with an icon and a remedy pointing at your server settings, for every account, on every sync — the connection check only ran when the sync daemon was not there to ask, which is rarely — while the daemon itself kept dialling every account on every tick and waiting out a fifteen-second timeout for each. Now a banner at the top of the window says "No internet connection", that your mail is still here and MailVault is showing what is already saved on this computer, and offers "Check again"; it clears itself the moment connectivity returns, with no click. While it is up, background syncing pauses rather than failing for every account, tick after tick. An error from a mail server only ever raises a suspicion: MailVault checks the internet itself before deciding you are offline, so one provider having a bad day does not make the whole app declare the network gone. Anything you ask for yourself — opening a message, deleting one — is never held back by the banner, because a mistaken verdict must not lock you out of an action you asked for. The old check also froze the window for up to four and a half seconds while it dialled three hosts one after another; the new one dials them together, off the main thread, and answers within a second and a half.
- **Mail links in other apps can open in MailVault.** Click an email address on a web page or in a document and the system hands it to whichever app is the default for mail; MailVault can now be that app. The link opens a compose window with the recipient, subject and body it carried already filled in — including when the click is what launches MailVault, where the link arrives before there is a window to show it in and would otherwise be lost. The first-run tour asks about it, and Settings gained a "Default email app" row that says whether MailVault opens mail links on this computer and, on Linux, offers to make it so. macOS only lets you choose this yourself, so there the row says where the switch lives — Mail's own Settings → General — rather than claiming a success the system refused.
- **A single message can be deleted from an open conversation, and collapsed messages say who they were sent to.** Finding one reply in a thread that should go used to mean leaving the reader and hunting for its row in the list, because the reader itself had no Delete for a message inside a conversation. Each expanded message now has one, behind the same confirmation the list uses; the message leaves the conversation and the reader stays open on the rest. Deleting a reply from the list while its conversation was open used to leave it in the reader as a ghost, and deleting the newest message closed the whole conversation — the reader now keeps up, and if the server refuses the delete, the message comes back. A message that exists only in your vault keeps its Delete in the row menu, as before. Collapsed messages in a conversation also now carry a "To:" line, so you can see who each one went to without expanding it.
- **An open message can be closed.** Opening a message was a one-way door: every other way out either opened something else — another row, another folder — or destroyed the message. In the stacked two-column layout that is the whole screen, because the list gives up more than half its height the moment a message opens, with no way to get it back. There is now a Close button in the reader's header, on a single message and on a conversation alike, and the list gets its space back.
- **Settings can open the folder your mail is stored in.** "Where your mail is stored" and "Second copy — external cold storage", both under Settings → Backup & Restore, each gained an Open Folder button: the first shows the vault in the Finder, the second opens whatever the field above it names — the external copy when one is configured, otherwise the app's own mail folder. A refusal prints under the row rather than leaving a dead button. The Mac App Store build used to leave the storage section out entirely, because that build cannot move the vault; it now shows the path and the button, and its users are the ones with the fewest other ways to find the folder, since a sandboxed app's files sit under ~/Library/Containers, which the Finder hides and Spotlight skips.
- **The first-run tour can set the look GraphicMeat recommends in one click.** The "Make it yours" step asks five questions — theme, layout, sidebar style, view and row density — before anyone has seen a message, and the answers only mean something once you have. A "GraphicMeat recommended" button sets all five to the combination the app is designed around — dark, three columns, tag cloud, list view, compact rows — the preview beside them changes as it does, and every one can still be changed afterwards, there or in Settings.
- **"Report a bug" now leads with the FAQ.** The dialog offered four ways to file something and no way to just get the answer. Its rows now read in the order most people need them: the FAQ, in the language the app is set to, then the existing public discussions, then a new report, then email as the private fallback, with the feature request last, since it is not a bug.
- **The website's docs and FAQ pages search the text of every page, and the FAQ names where your mail actually is.** The search box on each hub matched only the words in a question's title, so anything said in an answer was invisible to it. Both now search every guide, comparison, post and answer, and link straight to the section that matched; with scripting off, every page is still one link away. The docs index leads with the pages people arrive for — FAQ, features, changelog — groups the guides by topic, and its mobile menu works again. The FAQ's answer about where mail is stored gave the path a development build uses; the shipped app is sandboxed, so its data lives under ~/Library/Containers, which the Finder hides and Spotlight does not search. The answer now names that path and the two Linux ones, says why searching the drive finds nothing, and gives four ways to open the folder. A new answer says how often the folder list is re-read from the server.

### Fixed
- **A message's read state is now one fact, wherever MailVault keeps it.** Marking a message read or unread changed it on the server and on screen, but the copy in your vault kept whatever state it was stored with — and it was always stored as read, whether you had read it or not. That showed up in three places: a vault message rebuilt from its file could paint as unread when it was not, switching away from a folder and back could briefly bring the old state back until the next sync corrected it, and restoring a vault to a new server uploaded every message as read. A change made on another device never reached the vault or the external backup at all. Now the vault copy is stored with the message's real state, a change made in MailVault lands on the vault file, its external backup copy and every cache in one step, and every backup run carries over changes made elsewhere for the messages it already holds.
- **The Sent folder now syncs on servers that call it something else, and a moment's loss of network no longer costs every account its sync.** MailVault asked the server for the folder by the name it had been told — "Sent" — and Gmail, which serves it as "[Gmail]/Sent Mail", answered that no such folder exists, so that account's sent mail had never synced at all; a server that calls it "Sent Messages" failed the same way. When the server says a folder is missing, MailVault now looks the name up in the server's own folder list and syncs the folder the server actually serves; when the name was right, nothing changes. Separately, a network blip tore down the connection to every account at once and each lost its whole sync cycle; a connection that fails that way is now retried once. A rejected password is the server's answer and is never retried.
- **Deleting a message from the reading pane no longer puts it straight back, and a delete the server did not carry out is no longer reported as done.** The delete ran on a server connection MailVault had kept open and reused, and a connection the server had quietly closed while it sat idle answered the first command with "connection lost" within milliseconds: the row came back, and the second attempt worked because it happened to get a fresh connection. A delete now notices the dead connection and asks again on a new one — safe for a delete, because deleting a number the server has already removed does nothing. Writing the check for that found the worse half. MailVault read the server's answer to a delete, and to a flag change, only far enough to see that the command had been sent; a connection that died in the middle of the reply produced no error at all, so the delete was reported done — the row taken out, the count lowered, and the message stamped "deleted from server", the app's loudest claim about custody — over a message still sitting on the server. Every delete and flag change now waits for the server's own OK, and the reading pane says why when a delete is refused instead of silently restoring the row.
- **Deleting a message from the Inbox list no longer deletes a sent message that happened to share its number.** The Inbox list shows your sent messages alongside the received ones so a conversation reads whole, and a message's number is only unique inside its own folder. Ticking a received message and choosing Delete from server could delete the sent message under the same number instead: the row left the list, came back at the next sync, and the sent conversation lost a message. The selection now remembers which folder each ticked row came from, so the delete — and every other bulk action — reaches the message you ticked. Opening a sent message from the Inbox list also shows that message's own read state, not the received message's.
- **Marking a sent message unread from the Inbox list now changes that message, not another one.** The Inbox list shows the messages you sent alongside the ones you received, so a conversation reads whole. A message's number is only unique inside its own folder, and marking one of those sent copies unread sent the change to the Inbox message that happened to carry the same number: your sent copy kept its read state on the server and on screen, and an unrelated received message could flip instead. The change now follows the message's own folder — on the server, in the vault and in the list.
...
- **A folder no longer stays on "server copy not verified yet" after a sync that could not be waited for.** The vault glyph on every row says whether the server still holds its copy, and it can only say so once MailVault has listed the folder's messages on the server in full. When a folder opened from a cleared list and the background sync's answer never arrived — the sync daemon timed out, restarted, or spoke an older version of its protocol — MailVault fell back to asking the server directly, saw that nothing had changed since its last visit, and stopped there: correct about the folder, silent about the list it was showing, so every row kept the unverified wording until the next successful sync. The direct path now asks the server for the folder's message list the first time it finds nothing changed while that list is still unproven, one request per folder visit and none at all once it is known. The e2e run that caught this was testing against a sync daemon built a month earlier than the app, which is also fixed: the e2e build now stages the daemon it just built as the one the app launches, rather than whatever copy was staged last.
...
- **A folder shows what is already saved while its sync runs, and a refresh waits for the sync it asked for.** Asking the sync daemon to sync a folder and wait for the result could be answered with the result of the previous sync: once an account had synced once, every later wait was satisfied at once by the last answer, and one account's Inbox sync stood in for its Sent sync. Each sync now carries a ticket and a wait is for that sync alone, and a sync the daemon declines still answers rather than leaving the wait to time out. While a sync runs, the folder paints from what is saved on this computer and reloads when the sync lands, so an account with mail already on disk opens on that mail rather than on a spinner. Two smaller things from the same audit: the daemon writes each of its own files in one step, so being killed mid-write no longer leaves a torn file that the next save persists as empty; and a category you set on a message by hand is never overwritten by an automatic one that lands after it.
- **An exported message names its sender instead of "[object Object]".** Every export carried that placeholder wherever the sender belonged — in the image's filename, in the thread document's summary, contents list and participants line, and in the From, To and Cc rows of the header card drawn into the picture itself. The export read those fields as plain text where the rest of MailVault reads them as a name and an address; it now reads them the way the rest of the app does.
- **The keyboard steps through All Inboxes again, and its unread filter no longer hides the message you are reading.** In the combined list a row is known by its folder as well as its number, and the next and previous keys compared by number alone, so they never matched a row and did nothing. The unread filter compared the same way, so it dropped the open message the moment it was turned on. Both now ask the row where it lives.
- **"Delete everywhere" is no longer offered on a message MailVault holds no copy of, and the delete that replaces it says where the copies are.** The row menu ended with "Delete everywhere" on every row, including a message your vault had never held: there it did exactly what "Delete from server" one line above does, under a name claiming far more, behind a confirmation promising to clear a vault and a backup drive that had never held the message. The last item now appears only where MailVault holds a copy of its own, and it is named for where the message actually is — "Delete from vault", "Delete from vault & backup", "Delete from server and vault", and so on — so what the confirmation promises is what happens.
- **The tour's Premium list shows all ten features without a scrollbar.** The list was capped in height and scrolled, so the last two — "Export as image or HTML" and "Five devices" — existed only for someone who found a scrollbar nobody looks for on a first run.
- **A hovered sidebar row no longer touches the selected one.** The selected account or folder is marked by a wash of colour rather than a border, and the rows were drawn flush, so a hovered row's shading sat edge to edge against it and the two read as one striped block. There is now a little space between the rows of both lists.
...
- **"Archive All" on a conversation opened from All Inboxes now archives it, instead of failing every message.** The combined list stands in for every account at once, so it has no folder of its own; archiving read the folder off the list rather than off the message, and asked each server for a folder by a name no server has. Every message came back an error — "Archived with 4 error(s)" on a four-message thread — and nothing was saved. Each message is now archived in its own account and its own folder, which also fixes the same failure from the row's archive button, the row menu, the keyboard shortcut and "Archive selected" whenever the selection spanned more than one folder.

## [2.11.0] - 2026-08-31

### Added
- **The first-run tour now has a back button, and explains every mark MailVault can put on a message.** Going back a step was not possible at all: the six screens only moved forward, so a wrong language or a mistyped server meant starting again. There is now one back control, in the same place on every screen, and it respects the shortened tour someone gets when they already have an account. The screen about link and sender warnings used to name four things in words that appear nowhere else in the app; it now lists the warnings by the exact titles the alerts themselves use — "Dangerous links detected", "Sender impersonation detected", "Reply-To domain mismatch", "Tracking blocked" — and Settings → Security explains each one in full, beside a picture of the warning it opens, in every language MailVault speaks. That screen previously described only the two kinds of link warning, and did it in English no matter which language you had chosen, while saying nothing at all about faked senders, mismatched reply addresses or the tracking marks.
- **The Premium screens in the tour can be stepped through, and their screenshots open full size.** Ten features behind a list you had to aim at is a list most people scroll past; there are now previous and next buttons that wrap around, a position counter, and a click on any screenshot opens it large enough to actually read.
- **Double-click an account in the sidebar to go straight to its inbox.** One click on an account takes you back to whichever folder you were last reading in it, which is what you want most of the time and the wrong thing when what you actually want is new mail. Double-click now goes to that account's Inbox, from anywhere — another folder, another account, or the All Inboxes list.
- **Any message or thread can now be saved as a picture or as a single web page.** Mail is the one archive most people cannot hand to anyone else: forwarding it changes the headers, a screenshot cuts off at the window edge, and printing to PDF renders whatever the printer stylesheet felt like. MailVault now exports a message, or a whole conversation, as a PNG image of exactly what you were reading — or as one self-contained HTML file. The HTML folds every message in the thread behind a line giving its date and sender, so a forty-message conversation opens as a list you expand one message at a time, with the whole conversation listed down the left in date order — click any entry to jump to it — over a pair of buttons that unfold or fold all of it at once. It fits whatever window it is opened in, down to a phone, so there is never anything to scroll sideways; on a narrow screen the list moves above the conversation rather than squeezing it. There is nothing to fetch and no server to be alive: the single script in the file only sizes each message to the window and works those two buttons, and with scripting turned off the file still opens and every message still folds. Remote images are fetched once, during the export, and written into the file itself, so it still reads years later with the network unplugged — and every file says underneath which account and folder it came from, each message's identifier, and how many remote images were mirrored in, how many the sender's server would not give up, and how many tracking pixels were dropped rather than fetched. Nothing that can execute survives the export: an exported file is opened outside MailVault, where none of the protections around the reading pane apply, so scripts, frames and handlers are taken out before the bytes are written. A conversation too tall for one image is split into numbered pages rather than shrunk, because a soft picture is a worse record than a second file. Export sits in the open message's toolbar, in the thread header, in any row's menu and in the bar that appears when rows are ticked — and where a message's body is not loaded yet, MailVault fetches it the same guarded way the reading pane does, so a thread exports whole rather than half. This is a Premium feature; without a subscription the same button shows three samples MailVault renders on the spot with the very same code, which you can open or save to see exactly what you would get.
- **MailVault now speaks nine languages.** The whole app — every screen, every button, every message it puts on screen, the menu bar and the tray icon's menu, the notifications it sends and the errors it reports when a mail server refuses something — is available in English, Spanish, French, Italian, German, Brazilian Portuguese, Japanese, Korean and Simplified Chinese. Pick a language in Settings and it changes as you watch: nothing to restart, nothing to reinstall. Dates, times and numbers follow the language you chose rather than the one your computer happens to be set to, which matters if you read English mail on a German Mac, or the other way round. The website is translated too — every page, in all nine, with screenshots taken in each language rather than English pictures under translated text, so what the page shows is what you will see. Nothing about this reaches the network: the translations ship inside the app, and choosing one tells nobody anything.
...
- **MailVault now tells you when an email is watching you, and can stop it.** Most marketing mail carries a hidden image, usually a single transparent pixel, that your mail program fetches from the sender's server the moment the message opens. That one fetch reports back that you read it, at what minute, how many times you came back to it, roughly where you were, and what you read it on — none of which you agreed to. Every message MailVault opens is now checked for those beacons, and a small eye appears beside the subject, on the row and above the open message, next to the marks for a faked sender and a mismatched reply address. Click it and MailVault names the company doing the tracking and shows the exact address the pixel would have called. That much is free, for everyone, forever. With a subscription MailVault also takes the beacon out of the message before it is drawn, so nothing is ever requested and the sender is told nothing at all; the eye closes to say so. It works in the reading pane, in threads, in Chat view and in a popped-out message window, and there is a Tracker Blocking screen under Features that shows the same email side by side, tracked and cleared, with the tracking code itself spelled out. Nearly 500 known tracking addresses ship with the app — merged from the MailTrackerBlocker and Ugly Email projects and extended with senders newer than both — and the screen links out to both projects, with their licences, so you can read the lists rather than take our word for them. The list is bundled and updated in releases. MailVault never asks a server which pixels to block: doing that would be one more thing reporting on what you read.
- **MailVault now introduces itself the first time you open it.** A new install used to land on an empty mailbox list with an Add Account button and no explanation of what the app was for, which is a poor first impression for something whose whole point is that your mail stops living on someone else's computer. The first launch is now a short tour. It opens on the logo and asks which of the nine languages to use — asked, not guessed from the machine, because the language your computer is set to is often not the language you read mail in — and everything after that is already in it. Then it says, in four cards, what MailVault actually does with a mailbox and where the password goes: into the system keychain, not into a file the app writes. Adding a mailbox is one button on that page, and it opens the same account dialog as the sidebar does; there is also "I'll do this later" for anyone without their server details to hand, which carries on through the rest of the tour rather than dumping them at the end. Next it asks how the app should look, with a real message list beside the choice that changes as you pick — light, dark or following the system, and the density — so the decision is made by looking rather than by reading the names of the options. The last two pages say what is included for nothing and what a subscription adds, the second with pictures of each feature rather than a list of names, and anything there can be opened later from Settings -> Help without going through the tour again. While all of this is being read, MailVault is already fetching the first mailbox in the background, so finishing the tour lands on mail rather than on a spinner.
- **Settings -> Billing now says what a subscription buys.** It quoted a price against nothing at all: the features it pays for were described on five separate locked panels scattered through the app and on the website, and nowhere in the tab that asks for the money. Billing now lists every Premium feature above the plan card, each with a line saying what it does and, where it has a screen of its own, a link straight to it. The list appears in every build, including the Mac App Store one, because a list of features carries no price. Settings -> Help gained the same gallery, and a link to the FAQ in the language the app is set to.
- **The FAQ is now six pages instead of one.** Twenty-three questions on a single page meant scrolling past everything to reach anything. They are now grouped — getting started, storage and the vault, mail providers, Premium, backup and restore, and troubleshooting — behind a hub with a search box that filters the whole list of questions at once. Every answer is plain text that is always visible and needs no scripting to open, every page carries its own structured data so search engines can quote the answers, and old links into the single page still land on the right question.


### Fixed
- **The tour's two-column preview shows where a message is read.** Choosing the two-column layout drew a sidebar and a list and nothing else, which said the layout had nowhere to read a message. It has always had one — the reading pane sits under the list rather than beside it — and the preview now shows that.
- **Feature names in the Premium list are no longer cut off.** "Scheduled automatic backups", and the longer translations of it, ran past the edge of the button and were clipped mid-word, so the tile named a feature nobody could identify. Names now wrap.
- **The Continue button no longer moves while you browse Premium features.** The panel resized itself to whichever feature was showing, so the button slid up and down under the pointer.
- **Opening a FAQ topic no longer narrows the page.** The FAQ index and the topic pages disagreed about column width, so every click through visibly shrank the text.
- **The Reply-To warning is translated.** Its title was English in all eight other languages, unlike the two warnings either side of it.
- **A message that refused to open now opens on the first try.** Reading mail means asking the server for it, and MailVault keeps a few connections to your mail server open and reuses them rather than starting a new conversation for every message. A connection the server had quietly dropped in the meantime was still handed out, and the message opened onto "Server refused UID FETCH: connection lost". Pressing "Try again" worked — which made it look random — because the second attempt happened to get a fresh connection. MailVault now notices that the connection died under it and quietly asks again on a new one, so the first press is the only press.
- **"Mark as read" works on a message in your vault in the All Inboxes list.** Ticking a message the vault holds and choosing "Mark as read" did nothing at all — no error, no change, the row stayed bold. Messages the vault holds and the mail server does not are kept in a different list inside MailVault from the ones the server just sent, and every read/unread change only ever touched the second one. The change now reaches both, and it is written into the vault's own record of the message, so it is still read after switching folders or restarting rather than quietly coming back unread.
- **An account switch is no longer overtaken by the one before it.** Two switches in quick succession — which is exactly what a double-click is — could land on the first one's folder instead of the last one's. The switch that had already been abandoned still painted its own account and folder over the newer one on its way out, and then asked the server to refresh that folder as well. An abandoned switch now stops where it is told to.
...
- **Auto-cleanup rules now actually run — and every existing rule has been switched off until you say otherwise.** The rules that archive or delete mail past a certain age were written down by the settings screen in one shape and read by the engine in another: it looked for fields the screen has never saved, worked out an age threshold of nothing, and stopped before touching a single message. Every rule anyone has ever created has been silently doing nothing, run after unattended run. Simply fixing the reader would have been worse than the bug — rules made months ago, watched to do nothing and forgotten would have woken up on the next launch and started deleting real mail with no warning. So the stored rules have been rewritten into the one shape and every one of them switched off, with a notice on the screen saying why. Turning one back on is a single toggle, and that is yours to decide. A rule the engine cannot make sense of is now skipped and reported rather than treated as though everything were old enough to go.
- **The rule editor no longer offers a folder the engine refuses to touch.** Drafts could be picked, saved and switched on; the rule then sat there looking active and did nothing, because Drafts is protected and always has been. It is no longer in the list, and the two lists — what may be picked and what may be acted on — now come from the same place, so they cannot drift apart again.
...
### Changed
- **Every page on mailvaultapp.com is built from one menu definition.** The menu had been copied by hand into each page, so a new page froze it at whatever the page it was copied from happened to have — twice this left the price out of nearly every page. The release notes page was worse: it is rebuilt on every release from a template that carried its own copy, so publishing a version quietly reverted that page's menu. There is now a single definition every page is generated from, and the links move to the compact menu earlier on narrow screens, where six items in the longer languages had begun to overrun the bar.
...
- **The “Report a bug” dialog now takes what should exist, not only what is broken.** Someone who has just hit something wrong is the same person who knows what the app should have done instead, and that dialog is the only moment MailVault has their attention on the subject. It now offers “Suggest a feature” as its second row, filing into the Ideas section of the project’s discussions rather than Bug reports, and its heading says so. Email moved down below “Browse discussions”, because email is the private fallback and a public thread is the one the next person with the same problem can find — and the dialog now says what an email can cost: MailVault’s logs carry every address the app has seen, so a log is something you attach deliberately rather than by default. The dialog also carries a link to follow the project, and says who makes MailVault.
- **A locked feature now says what else Premium covers.** Every surface that asks for a subscription carried its own short pitch for the one thing it was gating and nothing about the rest, so seeing the whole list meant going looking for it. They now share a single “See everything in Premium” link out to the feature page, and one locked screen is enough. App Store builds do not show it, for the same reason they do not show the price: that page carries the web subscription’s price, and a Mac App Store build must not advertise a purchase made outside it.

## [2.10.4] - 2026-08-27

### Added
- **MailVault can now ask your mail server whether a message still exists anywhere on it.** A message you have archived is shown in gold — "your only copy" — when the vault on this computer is the last place it survives. That claim used to be worked out from the folder the message was archived from, which is not the same question: archive a message on your phone and it leaves the inbox for All Mail, a rule files it under a label, a delete puts it in the Bin. In every one of those the mail is perfectly safe and the row went gold anyway, so the claim was withdrawn — and with it the alarm worth having, the one that says someone else deleted your mail and the vault is what is left. Open a message you have archived and the band above it now offers "Check the server". MailVault searches every folder on the account for that exact message, and says what it found: still on the server and which folder has it, or gone from all of them. Only the second answer turns the row gold, and only when every folder actually answered — a folder that refuses to open is reported as a folder that did not answer, not as an empty one. The verdict is written next to the message on this computer, so it survives closing the app, and MailVault runs the search by itself when a message you have archived disappears from the folder it was archived from.
...
- **"Report a bug" now offers somewhere public to put it.** The button in the sidebar opened a message addressed to one inbox: every report was a private thread between one person and the developer, and the next person to hit the same thing had nothing to search. It now asks where the report should land. "Report on GitHub" opens a new thread in the Bug Reports section of the project's discussions, where it is searchable and where you are told when it is fixed. "Email the developer" is the message as before, with the app version, platform and account details already filled in, for anything you would rather not post. "Browse discussions" opens the list, because the answer may be there already.
- **Refer a friend.** A second button under it opens a new message with nothing filled in but the pitch: what MailVault does, the five things it is for, and a link to the site. Address it to someone and send.

### Changed
- **A "your only copy" message no longer paints its whole row gold.** The loudest surface in the app was spent saying something the row already said three other ways: the symbol beside the sender, the label on the open message, and the band above it. The gold wash also had to switch off the highlight that follows the pointer, so the one row you most wanted to click was the one row that did not respond to the cursor. The row now looks like every other row and behaves like one; the symbol, the label and the band still carry the claim.

### Fixed
- **A reply leaves from the mailbox you were reading.** Reply, Reply All and Forward opened on whichever account had sent the last message, not the one holding the message being answered — so an answer to mail in one mailbox went out under another mailbox's address, to a correspondent who had never written to it. A message opened from a mail server carries no record of which mailbox it came from, and that missing record was what the choice fell back on. The account the message is in is now kept with it, and reply and forward leave from that account. Only a brand-new message still opens on the account you last sent from.
- **The gold "only copy" mark stops going out — after an archive, and in search results.** The mark that says the copy in your vault is the last one left is worked out from a record kept beside each message on this computer, and that record was not travelling with the messages. Two things followed. Saving, archiving or deleting anything in a folder made MailVault rebuild that folder's list from the stored messages alone, so the gold went out on every proven message in the folder — silently, until something else happened to reload them. And search results were built the same way, so a message MailVault had proved was the last copy left showed gold in its folder and plain grey one search away, which is the wrong way round: search is where you look for a message you are afraid has gone. The record now travels with the messages however the list is built, and a search result carries the same mark as the row it came from, whether you search one folder or all of them.
- **A backup that saved almost everything no longer reports itself as failed.** If a mail server refused to hand over a single message — one that has been sitting there unread for years, one the server has trouble reading back — the whole run was reported as a failure, and the notification that arrived said "Backup failed" followed by the words "Unknown error", because nothing was passed along to say what had actually gone wrong. On 27 August that was a run which had saved 788 of 789 messages. A run that saves what the server gives it is now reported as what it is: the account is marked incomplete rather than failed, the notification says "Backup partially complete" and carries the count and the server's own words for the message that did not arrive, and the backup card in Settings shows the same. A run that saves nothing is still a failure, and still retries.
- **MailVault says when Premium stops applying to this computer.** A subscription can end between one check and the next — it lapses, a payment fails, or the device loses its place to another machine. Until now nothing was said: the Billing tab quietly went back to reading "Free", the features that need Premium stopped, and the only way to find out was to notice something missing. MailVault now says so plainly in Settings → Billing, explaining which of those happened, and sends a notification so the message arrives even when Settings is not open. Automatic backups are deliberately left running — billing is never a reason to stop copying mail onto your own disk.
- **Automatic backups no longer run without a subscription.** Asked by someone who wanted to know why backup notifications kept arriving on an install that had never signed in to a paid account. The per-account backup switches were gated; the "Automatic Backup" master switch above them was not — and that switch is an override that turns on every visible account at once, so a single flip on a free install backed up everything and announced each run, notifications being on by default. The check now sits on the one door every automatic run passes through, whichever of them wakes it. Manual "Back up now" is unchanged and still free. A schedule left behind by a subscription that has lapsed is kept rather than deleted: it stops running, and starts again untouched if you resubscribe.
- **The dot that means "your backup drive has this" no longer looks like the mark for "saved in your vault".** Every row in the list carries a small symbol saying where that message lives: a cloud for one that is only on the mail server, a disk for one saved in your vault, and a different symbol again for the last copy left anywhere. A tiny dot beside that symbol says your external backup drive has a copy too. The dot was painted in the same green MailVault uses for the vault itself, so a message that was only on the server showed a green dot directly above the words "Not saved to your vault yet" — the colour said one thing and the sentence said the opposite. The dot now carries no colour of its own. Filled means the drive has it, hollow means the drive is not connected and there is no answer either way, and the colour belongs to the symbol it sits on.
- **An open message now says the same thing as the row you clicked to open it.** The band above a message worked out where that message lives without ever asking about the backup drive, while the symbol beside the sender's name a couple of centimetres below it did ask. One could read "On the server" while the other read "On the server and backup drive", about the same message, at the same moment. Both now read the same record, through the same piece of MailVault. The band also names the vault in its first line rather than only in the small print: a message can be on your mail server and on your backup drive and still not be in your vault, and that is the fact worth reading first.
- **A message in Sent stops borrowing an inbox message's backup.** Mail servers number messages within one folder at a time, so number 8 in your inbox and number 8 in Sent are two different messages. MailVault matched copies on the backup drive by that number alone — and the inbox view shows your replies from Sent inside its conversations, so a reply that had never been backed up could wear the mark of an unrelated inbox message that had. Backup copies are now matched by folder as well as by number, MailVault reads the Sent folder's backups alongside the folder you are looking at, and a message in a folder it has not read is reported as unknown instead of as "not on your backup drive" — an answer nobody had asked for.
- **MailVault stops mentioning a backup drive you never set up.** The small dot beside each message's symbol has three states: filled when your backup drive holds a copy, hollow when the drive is not connected and there is no answer either way, and absent when there is no copy on it. MailVault could not tell "the drive is unplugged" apart from "there is no drive", so every message belonging to someone who had never chosen a backup location wore the hollow dot, and the message you opened said "Backup drive not connected — can't verify" — a complaint about a feature that had never been turned on. With no backup location configured, nothing is drawn and nothing is said. A drive that has been set up and is not reachable still reports itself as exactly that.
...
### Fixed
- **A message in a conversation stops showing its subject where its text should be.** MailVault keeps a copy of every message you open on this computer, filed under the number its mail server gave it. Those numbers are only meaningful to one server: move a domain to a new mail host and the new server hands the same numbers to different messages, so the copy filed under a number is somebody else's mail. MailVault noticed that and gave up — and the conversation then printed the message's own subject line, in italics, exactly where the message should have been. It reads as an email one line long. MailVault now asks the server for the message you actually clicked, and if that fails too it says the message could not be loaded rather than quietly showing you something that looks like the real thing.
- **"View Source" shows the message you are looking at.** The raw source was read straight out of the stored copy filed under that number, with nothing checking it was the right message — so after a change of mail host it could hand over another message's headers and body under the header of the one on screen. The source is now checked against the message it claims to be, and MailVault says so plainly instead of showing the wrong one.
- **A backup no longer undoes MailVault's own repair of your archive.** When a mail server reissues its message numbers, MailVault repairs the archive on this computer: mail the server still has is refiled under its new number, and mail it does not have is set aside — kept on disk, never deleted. The next backup then copied those set-aside messages back off the backup drive into the archive, under the numbers they used to have, undoing the repair. Every backup did it again, and nothing showed that it had happened. Messages the repair set aside now stay set aside, while a copy it never ruled out still comes back from the backup drive as before. An archive that was already affected keeps its wrong copies until the next repair runs.
- **A bulk archive or delete now says what it is doing to a screen reader.** Moving thousands of messages off a server reported its progress in a bar and nowhere else, so with a screen reader on, the longest and least reversible operation in MailVault ran in silence. Bulk save, bulk operations and the selection bar now announce the phase, each quarter of the way through, and how it ended — including how many failed — without reading out every percent for the length of the run.
...
- **Folder names with accented letters read the way they are spelled.** A folder called "Bokelmühle" showed up as "Bokelmu&Awg-hle" — in the sidebar, above the message list, in the folder pickers, and in the backup coverage list. Mail servers send folder names in an escaped form, and MailVault was printing that form as-is instead of turning it back into letters. Every place a folder is named now shows the real name, whatever alphabet it is written in: äöüå, accents, Cyrillic, Japanese. Nothing on disk changed — a folder in your archive is still stored under the name the server gives it, so a backup made before this update restores exactly as it did.
...
### Fixed
- **A quoted reply that arrives without a marker is folded away like any other.** When someone replies to their own message — Fastmail does this, and it is common on a phone — the message being quoted arrives with nothing marking it as a quote, so MailVault showed all of it under the reply and the same conversation ran twice down the page. That quote is now recognised by its "Original Message" header and tucked behind the "…" button, with the From, Date and Subject lines left in view, exactly like the quotes every other mail app sends. Quoted plain-text mail written the same way folds too.
- **A message you send keeps the spacing the compose window showed.** Paragraphs went out with no spacing of their own, so the mail app at the other end — MailVault's own reading pane included — filled in its default and a message written with single blank lines arrived looking double-spaced. The spacing now travels with the message, so what you see while writing is what lands.
- **A message with a very long subject no longer pushes the app off the screen.** Opening a conversation whose subject runs on in one unbroken line — a delivery report, say — stretched the app wider than its own window: the right-hand side of the message was cut off with no way to reach it, and selecting any text slid the whole window sideways and would not slide back, leaving the message list off-screen so nothing else could be opened. The layout now fits the window whatever the subject, an over-long subject is shortened with an ellipsis as it always should have been, and no part of the app can be scrolled out of reach.
...
- **Outgoing mail now carries a properly formed Message-ID.** Every message MailVault sent was missing the angle brackets the mail standard requires around its Message-ID. Some receiving servers and spam filters count a malformed header against a message, and other mail apps key replies to that header, so the conversation could fail to line up on the other end. The header now goes out in the standard form, and the copy filed in Sent still matches it.
- **"Send Mail As" no longer suggests people you were merely copied alongside.** The suggestion chips under the field mined the To and Cc lines of received mail, so a mailbox that is often copied with the same colleagues was offered their addresses as aliases. Being on a thread is no proof that mail to an address lands in this mailbox, so that source is gone: the chips now list only addresses this account has actually sent from, under "You've sent as:".
- **Premium prices are quoted in the currency you will actually be charged, on every screen.** The panels for locked features worked the currency out from your computer's language settings, while the Billing tab — and the checkout that takes the money — works it out from where you are. A machine set to British English inside the eurozone was offered "£3.50/month or £21/year" on the Migration panel and "€4/mo" on the plan card two clicks later, then billed in euro. Every price in the app now comes from the same place as the charge, and is remembered so it is right the moment a panel opens. The pricing, home, features and FAQ pages on the website do the same, instead of quoting US dollars to everyone.
...
- **An unread-only filter for the message list.** The envelope button in the list header hides everything you have already read; press it again to bring it back. Conversations stay in the list while any message in them is unread, the message you currently have open stays put even once it is marked read, and the header says how many unread messages it is counting — including how much of a large mailbox has been loaded so far, since the filter can only see mail MailVault has already fetched. When nothing is unread the list says so and offers a way back to all messages.
...
### Fixed
- **The selection bar and Bulk Email Operations no longer disagree about how much is selected.** With 65 emails picked, the modal said "65 emails selected" while the bar along the bottom said "52 selected (65 emails)" — it led with the number of conversations those emails belong to, not the number of emails every button on it acts on. The bar now leads with the message count and names the conversation count for what it is: "65 selected (52 conversations)". The conversation count was also short whenever the selection reached mail further back than the list had loaded, which a date range in bulk operations routinely does; those messages are now counted as well.
- **Outlook mail saved on this computer is shown newest-first again, and is no longer thrown away.** Outlook gives its messages no number of their own, so MailVault assigns one the first time it sees each message — working through the folder newest first, which means the oldest mail ends up with the highest numbers. Two things went by those numbers as though they ran the other way. Opening an Outlook account showed the saved copies with the highest numbers, so the list filled with the oldest mail in the folder until the server answered. And the check for messages deleted from the server treated everything below the newest page as gone: refreshing a folder MailVault held more of than one page could delete hundreds of saved messages that were still on the server. Both now go by the date on the message, and mail whose date cannot be read is left alone rather than removed.
- **A conversation's subject is back in the email list.** Rows that group several messages together showed the participants, the message count and the date but no subject at all at the standard list width — the subject was squeezed to nothing rather than shortened, so the row read as mail with no subject. Widening the list brought it back. Conversation rows now keep room for the subject and shorten the participants instead, the same way single-message rows already did.
- **Premium prices in the app now match what you are actually charged, in your own currency.** The panels for locked features quoted "$3/month or $25/year" — a price that changed months ago — and quoted it in dollars no matter where you are. They now show the price for your machine's region: pounds in the UK, euros across the eurozone, dollars in the US, and euros elsewhere, which is what checkout bills. Prices ending in a half unit, such as £3.50, are no longer written as "£3.5" on the Billing tab.
...
- **A message no longer shows mail from the account's previous email provider.** After moving a mailbox to a different provider, the new server hands out its own message numbers, and a message MailVault had already saved to this computer could be filed under a number the new server had since given to something else. Opening the row showed the old, saved message in full — its sender, its date, its subject and its body — with nothing to say anything was wrong. Every saved copy is now checked against the message it is opened for, and a copy that turns out to be a different message is ignored in favour of the server's.
- **"Delete everywhere" no longer reports success on an Outlook message it did not delete.** Outlook messages are addressed by their position in the folder listing, which shifts every time mail arrives or leaves, so MailVault could lose track of which message a row pointed at. When that happened the delete was skipped, the row disappeared anyway, and the message came back on the next reload. MailVault now uses an address carried by the row itself, re-reads the folder when that is not enough, and when it still cannot identify the message it reports the delete as failed and leaves the row — and every local copy — where it is. The same fix applies to deleting, moving, and marking Outlook mail as read.
- **An Outlook message now keeps the same number, so the copy saved to this computer stays attached to the message it came from.** Outlook gives its messages no number of their own, so MailVault used each message's position in the folder — which moves every time mail arrives or is deleted. A single new email could shift every message down by one, and the copy saved to this computer was then filed under a number that already belonged to something else: the same email could be saved twice under two numbers, while a third number pointed at mail it never held. MailVault now gives each Outlook message a number of its own the first time it sees it and keeps it for good, so the saved mail, the email list and the folder stay in agreement. Accounts already set up keep the numbers they have.
