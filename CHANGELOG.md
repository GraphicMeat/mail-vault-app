...
- **Archiving a message no longer takes the gold mark off every other message in that folder.** The mark that says the copy in your vault is the last one left is worked out from a record kept beside each message on this computer. Saving, archiving or deleting anything in a folder made MailVault rebuild that folder's list from the stored messages alone, without that record — so the gold went out on every proven message in the folder, silently, until something else happened to reload them. The record now travels with the messages however the list is rebuilt.
- **A message found by search now carries the same gold "only copy" mark it has in its folder.** When MailVault knows the copy in your vault is the last one left — you deleted the copy on the server, the message was written here and never sent to one, or a search of every folder on the server came back empty — the message is shown in gold with a crossed-out cloud, so the one thing you cannot lose twice is impossible to miss. Search results were built without that record and were drawn as ordinary archived mail, so the same message was gold in its folder and plain grey one search away — the wrong way round, because search is where you look for a message you are afraid has gone. Results now carry the proof and are marked with it, whether you search one folder or all of them.
