...
// MailVault patch: every string in a BODYSTRUCTURE is read with `string_lossy`
// / `nstring_lossy` instead of the `*_utf8` twins. These are filenames, ids and
// descriptions — one non-UTF-8 byte in a `("NAME" "R\xE9sum\xE9.pdf")` param
// used to fail the whole FETCH line, and async-imap answers a failed line by
// killing the connection: no mail at all, over a mojibake attachment name.
// ENVELOPE keeps byte `nstring`, so it was never affected.
...
fn body_encoding(i: &[u8]) -> IResult<&[u8], ContentEncoding<'_>> {
    alt((
        ...
        map(string_lossy, ContentEncoding::Other),
    ))(i)
}
...
fn body_lang(i: &[u8]) -> IResult<&[u8], Option<Vec<Cow<'_, str>>>> {
    alt((
        ...
        map(parenthesized_nonempty_list(string_lossy), Option::from),
    ))(i)
}
...
fn body_extension(i: &[u8]) -> IResult<&[u8], BodyExtension<'_>> {
    alt((
        ...
        map(nstring_lossy, BodyExtension::Str),
        map(
...
mod tests {
    ...
    fn test_body_lang_data() {
        // A closing quote at the very end of the input is Incomplete now — see
        // `quoted`. On the wire a quoted-string always has a follower.
        assert_matches!(
            body_lang(br#""bob")"#),
            Ok((b")", Some(langs))) => {
    ...
    fn test_body_extension_data() {
        assert_matches!(
            ...
            Ok((b")", BodyExtension::Str(Some(Cow::Borrowed("blah")))))
        );
    ...
    #[test]
    fn a_latin1_filename_param_parses_lossily() {
        // MailVault patch. Raw 0xE9 where UTF-8 wants two bytes — Apple Mail
        // and older MUAs both emit this.
        const BODY: &[u8] = b"(\"APPLICATION\" \"PDF\" (\"NAME\" \"R\xe9sum\xe9.pdf\") NIL NIL \"BASE64\" 100 NIL (\"attachment\" (\"FILENAME\" \"R\xe9sum\xe9.pdf\")) NIL NIL)";

        assert_matches!(
            body(BODY),
            Ok((_, BodyStructure::Basic { common, .. })) => {
                let params = common.ty.params.expect("NAME param");
                assert!(params[0].1.contains('\u{FFFD}'), "got {:?}", params[0].1);
                assert!(params[0].1.ends_with(".pdf"), "got {:?}", params[0].1);
            }
        );
    }
